...
public:
    ...
    marker_frame("red/base_footprint")
  {
    ...
    laser_sub = create_subscription<sensor_msgs::msg::LaserScan>(
      ...
      std::bind(&Landmarks::lidar_callback, this, std::placeholders::_1));
        ...
        for (size_t index = 1; index < clusters.at(mark).size(); index++) {
          if (sqrt(
              pow(
                ...
                2) + clusters.at(mark).at(index) * sin(clusters.at(mark).at(0) * angle_increment)) >
            0.15)
          {
            cluster_cyl.markers.at(mark).points.push_back(
