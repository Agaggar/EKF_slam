...
private:
  ...
  void cluster_callback(visualization_msgs::msg::MarkerArray clus)
  {
    ...
    for (size_t loop = 0; loop < clus.markers.size(); loop++) {
      ...
      if (clus.markers.at(loop).action == 0 && clus.markers.at(loop).points.size() >= 4 &&
        actual_circles.markers.size() > 0)
      {
        ...
        if (circle.at(2) < 0.1 &&
          distance(circle.at(0) + means.at(0), circle.at(1) + means.at(1)) > .1 &&
  ...
  mat shiftPoints(mat data_points, std::vector<double> centroid)
  {
    return arma::join_rows(
      (data_points.col(0) - centroid.at(0)),
      (data_points.col(1) - centroid.at(1)));
  }
  ...
  {
    return {-A(1) / 2 / A(0), -A(2) / 2 / A(0), sqrt(
        (A(1) * A(1) + A(2) * A(2) - 4 * A(0) * A(
          3)) / (4 * A(0) * A(0)))};
  }
  ...
  double RMSE(std::vector<double> circle, vec xcoor, vec ycoor)
  {
    ...
    for (size_t i = 0; i < xcoor.n_elem; i++) {
      sum +=
        pow(
        ((xcoor(i) - circle.at(0)) * (xcoor(i) - circle.at(0)) + (ycoor(i) - circle.at(1)) *
        (ycoor(i) - circle.at(1)) - circle.at(2) * circle.at(2)), 2);
    }
  ...
  std::vector<double> computeStats(vec x_coor, vec y_coor)
  {
    ...
    for (size_t loop = 1; loop < (x_coor.n_elem - 1); loop++) {
      P1 = {(x_coor(indices(0)) - x_coor(indices(loop))), (y_coor(indices(0)) - y_coor(
          indices(
            loop)))};
      P2 =
      {(x_coor(indices(x_coor.n_elem - 1)) - x_coor(indices(loop))),
        (y_coor(indices(x_coor.n_elem - 1)) - y_coor(indices(loop)))};
