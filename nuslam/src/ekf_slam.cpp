/// \file
/// \brief implementing ekf slam, following Matt Elwin's notes here: https://nu-msr.github.io/navigation_site/lectures/slam/slam.pdf
///
/// PARAMETERS:
///     rate (double): frequency of timer callback (hertz), defaults to 200
///     timestep (size_t): iterations of timer callback
///     min_num_associate (size_t): how many times an obstacle must be seen before it can be associated
...
///     max_range (double): max_range for mahalanobis distance
///     greenbot (turtlelib::DiffDrive): diff drive object of green robot
///     use_lidar (boolean): determnie whether to use lidar (and thus, data associate)
/// PUBLISHES:
...
///     ~/circle_clusters (visualization_msgs::msg::MarkerArray): clusters that are also circles
/// SERVERS:
///     none
/// CLIENTS:
///     none
...
public:
    ...
    use_lidar(false)
  {
    ...
    cluster_sub = create_subscription<visualization_msgs::msg::MarkerArray>(
      "~/circle_clusters", 100,
      std::bind(&Ekf_slam::fake_sensor_callback, this, std::placeholders::_1)
    );
...
private:
  ...
  vec qt, qt_minusone, dq, mt_minusone, mt, zeta_update, zeta_predict, zeta_prev, m_seen, zjt,
    zhat_jt, dz, landmark_temp, num_associated_landmark, d_zeta_prev;
  ...
  {
    if (timestep == 0) {
      m_seen.ones(poss_obs);   // have you seen the obstacle
      m_seen = -10 * m_seen;
      create_all_cylinders();
      qt.zeros(3);   // odom state at time t
      qt_minusone.zeros(3);   // odom state at t before slam
      bigAt.zeros(3 + 2 * poss_obs, 3 + 2 * poss_obs); // At matrix from slam
      qt =
      {greenbot.getCurrentConfig().at(2), greenbot.getCurrentConfig().at(0),
        greenbot.getCurrentConfig().at(1)};
      qt_minusone =
      {greenbot.getCurrentConfig().at(2), greenbot.getCurrentConfig().at(0),
        greenbot.getCurrentConfig().at(1)};
      mt.zeros(2 * poss_obs);
      landmark_list.zeros(poss_obs, 2);   // keep track of relative locations of landmarks, useful for false positive
      sys_cov_minusone.zeros(3 + 2 * poss_obs, 3 + 2 * poss_obs);
      bigI.eye(3 + 2 * poss_obs, 3 + 2 * poss_obs);
      bigN = 0;   // how many obstacles have I seen
      ...
      zeta_predict = arma::join_cols(qt, arma::vec(2 * poss_obs, arma::fill::zeros)); // slam state predicted
    ...
    js_green.velocity =
      std::vector<double>{wheel_rad.at(0) - greenbot.getWheelPos().at(0),
      wheel_rad.at(1) - greenbot.getWheelPos().at(1)};
  ...
  void js_callback(sensor_msgs::msg::JointState js)
  {
    if ((js.position.size() > 1)) {
      ...
      greenbot.fkinematics(
        std::vector<double>{wheel_rad.at(0) - greenbot.getWheelPos().at(
            0), wheel_rad.at(1) - greenbot.getWheelPos().at(1)});
    }
    qt =
    {greenbot.getCurrentConfig().at(2), greenbot.getCurrentConfig().at(0),
      greenbot.getCurrentConfig().at(1)};                                                                             // current odom
  }
  ...
  void fake_sensor_callback(visualization_msgs::msg::MarkerArray obs)
  {
    /// PREDICT step
    dq =
    {turtlelib::normalize_angle(qt(0) - qt_minusone(0)), qt(1) - qt_minusone(1),
      qt(2) - qt_minusone(2)};
    ...
    for (size_t landmark_id = 0; landmark_id < obs.markers.size(); landmark_id++) {
      if (obs.markers.at(landmark_id).action != 2) {
        ...
        phij =
          (atan2(
            obs.markers.at(landmark_id).pose.position.y,
            obs.markers.at(landmark_id).pose.position.x));
        if (m_seen.at(ell) != obs.markers.at(landmark_id).id) {
          // RCLCPP_INFO(get_logger(), "now you see landmark: %ld", ell);
          m_seen(ell) = obs.markers.at(landmark_id).id;
          zeta_predict(3 + 2 * ell) = zeta_predict(1) + rj * cos(phij + zeta_predict(0)); // update measured landmark x
          zeta_predict(4 + 2 * ell) = zeta_predict(2) + rj * sin(phij + zeta_predict(0)); // update measured landmark y
        }
        // actual measurement (comes from red robot)
        zjt = {rj, phij};
        measurement_model(ell);
        dz = zjt - zhat_jt;
  ...
  /// \brief data association function algorithm
  /// \param cyl_x - measured obstacle's x coordinate
  /// \param cyl_y - measured obstacle's y coordinate
  /// \return associated landmark id
  size_t dataAssociate(double cyl_x, double cyl_y)
  {
    if (bigN >= poss_obs) {
    ...
    // RCLCPP_INFO(get_logger(), "code should not reach here %ld, %.6f", landmark_maha, dstar);
    return bigN;
  }
  ...
  nav_msgs::msg::Odometry compute_odom()
  {
    ...
    turtlelib::Twist2D vb{d_zeta_prev(0) - zeta_predict(0), d_zeta_prev(1) - zeta_predict(1),
      d_zeta_prev(1) - zeta_predict(1)};
  ...
  {
    for (size_t loop = 0; loop < bigN; loop++) {
      all_cyl.markers.at(loop).header.stamp = get_clock()->now();
      if (turtlelib::almost_equal(
          getLandmarkX(loop), 0.0,
          1e-6) && turtlelib::almost_equal(getLandmarkY(loop), 0.0, 1e-6))
      {
        // if (m_seen(loop) >= 0.0) {
