...
private:
  ...
  double x_length, y_length, input_noise, slip_fraction, basic_sensor_variance, max_range,
    collision_radius, encoder_ticks_per_rad, motor_cmd_per_rad_sec;
  bool draw_only;
  double angle_min, angle_max, angle_increment, time_increment, scan_time, range_min, range_max,
    lidar_noise;
  ...
  std::normal_distribution<> gauss_dist_vel_noise, gauss_dist_obstacle_noise,
    gauss_dist_lidar_noise;
  std::uniform_real_distribution<> unif_dist;
  double relative_x, relative_y, a, b, c, disc, x_int1, x_int2, y_int1, y_int2, meas_samp_x,
    meas_samp_y, d1, d2, theta_collision;
  ...
  void update_sd()
  {
    ...
    redbot.setWheelPos(
      std::vector<double>{phiprime_noise.at(0) + redbot.getWheelPos().at(
          0), phiprime_noise.at(1) + redbot.getWheelPos().at(1)});
  ...
  void checkCollision()
  {
    for (size_t loop = 0; loop < (all_cyl.markers.size() - 2); loop++) {
      dist_cyl = distance(
        redbot.getCurrentConfig().at(0), redbot.getCurrentConfig().at(
          1), all_cyl.markers.at(loop).pose.position.x, all_cyl.markers.at(loop).pose.position.y);
      if (dist_cyl <= (collision_radius + cyl_radius)) {
        ...
        obs_collide =
        {robo_collide.x - all_cyl.markers.at(loop).pose.position.x,
          robo_collide.y - all_cyl.markers.at(loop).pose.position.y};                                                                         // magnitude is dist from robot to obstacle
  ...
  void cylinders_as_measured()
  {
    ...
    Trw =
    {{redbot.getCurrentConfig().at(0), redbot.getCurrentConfig().at(1)},
      redbot.getCurrentConfig().at(2)};
    Trw = Trw.inv();
    for (size_t loop = 0; loop < all_cyl.markers.size(); loop++) {
      if (distance(
          relative_x, relative_y, all_cyl.markers.at(loop).pose.position.x,
          all_cyl.markers.at(loop).pose.position.y) > max_range ||
        distance(
          relative_x, relative_y, all_cyl.markers.at(loop).pose.position.x,
          all_cyl.markers.at(loop).pose.position.y) < range_min)
      {
        ...
        obst_r = Trw(
          turtlelib::Vector2D {all_cyl.markers.at(
              loop).pose.position.x, all_cyl.markers.at(loop).pose.position.y});
        measured_cyl.markers.at(loop).pose.position.x = obst_r.x + gauss_dist_obstacle_noise(
          get_random());
        measured_cyl.markers.at(loop).pose.position.y = obst_r.y + gauss_dist_obstacle_noise(
          get_random());
    ...
    for (double sample = angle_min; sample < angle_max; sample += angle_increment) {
      ...
      for (size_t check = 0; check < (walls_x.points.size() + walls_y.points.size()); check++) {
        range_lidar.push_back(
          check_incidence(
            x0, y0, meas_samp_x, meas_samp_y, poss_collision.at(check).at(0),
            poss_collision.at(check).at(1), 0.001));
      }
      // check dist to obstacles
      for (size_t check = (walls_x.points.size() + walls_y.points.size());
        check < poss_collision.size(); check++)
      {
        range_lidar.push_back(
          check_incidence(
            x0, y0, meas_samp_x, meas_samp_y, poss_collision.at(check).at(0),
            poss_collision.at(check).at(1), cyl_radius));
      }
  ...
  float check_incidence(
    double Rx, double Ry, double Mx, double My, double obs_x, double obs_y,
    double r)
  {
    a = 1 + (My - Ry) / (Mx - Rx) * (My - Ry) / (Mx - Rx);
    b = 2 * (-obs_x + (-(My - Ry) / (Mx - Rx) * Rx + Ry - obs_y) * (My - Ry) / (Mx - Rx));
    c =
      (obs_x * obs_x - r * r + ((My - Ry) / (Mx - Rx) * Rx - Ry + obs_y) *
      ((My - Ry) / (Mx - Rx) * Rx - Ry + obs_y));
    disc = b * b - 4 * a * c;
      ...
      if (disc >= 0.001) {
        ...
        if (d2 < max_range) {
          if (d2 < d1 && ((x_int2 - Rx) / (Mx - Rx) > 0) && ((y_int2 - Ry) / (My - Ry) > 0)) {
            return d2;
          } else if (d1 < max_range && ((x_int1 - Rx) / (Mx - Rx) > 0) &&
            ((y_int1 - Ry) / (My - Ry) > 0))
          {
            return d1;
          }
  ...
  void create_walls()
  {
    ...
    for (size_t loop = 0; loop < walls_x.points.size(); loop++) {
      poss_collision.push_back(
        std::vector<double>{walls_x.points.at(loop).x, walls_x.points.at(
            loop).y});
    }
    for (size_t loop = 0; loop < walls_y.points.size(); loop++) {
      poss_collision.push_back(
        std::vector<double>{walls_y.points.at(loop).x, walls_y.points.at(
            loop).y});
    }
  }
